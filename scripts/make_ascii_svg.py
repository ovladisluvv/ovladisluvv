...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "ovladisluvv-ascii.svg")

COLS = 100
ROWS = 53
CELL_W = 8
CELL_H = 15
RAMP = " `.:-=+*#%@"  # bright(sparse) -> dark(dense); leading space clears bg

# the prepped image already has bg removed + CLAHE local contrast, so only
# light global tuning is needed here.
CONTRAST = 1.08
BRIGHTNESS = 1.06
GAMMA = 0.90        # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
PRE_BLUR = 3.0       # подавляет мелкий рисунок рубашки
ZOOM = 1.20          # приближает человека на 20%
...
# Центральный квадратный crop с привязкой к нижнему краю.
# Убирает пустое пространство сверху и часть кресла по краям,
# но сохраняет пропорции изображения.
width, height = im.size

crop_side = max(1, int(min(width, height) / ZOOM))
crop_left = max(0, (width - crop_side) // 2)
crop_top = max(0, height - crop_side)
...
# Не повышаем резкость: она усиливает рисунок рубашки.
if SHARPEN:
    im = im.filter(
...
# Лёгкое подавление мелкой текстуры до уменьшения.
if PRE_BLUR > 0:
    im = im.filter(
        ImageFilter.GaussianBlur(radius=PRE_BLUR)
    )

im = ImageEnhance.Brightness(im).enhance(BRIGHTNESS)
im = ImageEnhance.Contrast(im).enhance(CONTRAST)

# BOX усредняет область каждого ASCII-символа.
# LANCZOS может сохранять лишнюю рябь и контуры ткани.
im = im.resize(
    (COLS, ROWS),
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">ovladisluvv@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'ovladisluvv@github:~$ whoami <tspan fill="{INK}">Vladislav Ogai</tspan></text>')
